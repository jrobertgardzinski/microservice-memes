...
/**
 * Casts a voter's vote on a comment — the same one-vote-per-voter rule as memes. Returns the
 * comment's new score, or empty when there is no such comment under that meme.
 */
public class VoteOnComment {
    ...
    public Optional<Integer> execute(String memeId, String commentId, String voter, VoteDirection direction) {
        return commentRepository.find(commentId)
                ...
                .map(comment -> {
                    ...
                    return commentVoteRepository.scoreOf(commentId);
                });
    }
}
